...
print(f"Bounds High: {upper_bound} Low: {lower_bound}")
...
def get_actor():
	...
	#Lunar Lander Engines are off in range -0.5 to 0.5

	inputs = layers.Input(shape=(num_states,))
	out1 = layers.Dense(512, activation="relu")(inputs)
	out2 = layers.BatchNormalization()(out1)
	out3 = layers.Dense(256, activation="relu")(out2)
	out4 = layers.BatchNormalization()(out3)

	outputs = layers.Dense(2, activation="tanh", kernel_initializer=last_init)(out4)
	# As it outputs the action so 2 neurons at the end
...
'''
# last_actor = load_model('lunar_actor.h5')
# last_critic = load_model('lunar_critic.h5')
	Actually we saved weights not model, this would have worked of we do
	model.save('my_model.h5')
'''
# Let us use last saved weights
# actor_model.load_weights('lunar_actor.h5')
# critic_model.load_weights('lunar_critic.h5')
...
# Let us use last saved weights
# target_actor.load_weights('lunar_target_actor.h5')
# target_critic.load_weights('lunar_target_critic.h5')
...
for ep in range(total_episodes):
    ...
    while True:
        ...
        # action = policy(tf_prev_state, None)
...
# Save the weights
actor_model.save_weights("lunar_actor.h5")
critic_model.save_weights("lunar_critic.h5")

target_actor.save_weights("lunar_target_actor.h5")
target_critic.save_weights("lunar_target_critic.h5")
